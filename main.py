...
def load_and_split_pdfs(folder_path):
    """ Load PDFs from folder and split to chunks"""
...
order_vectordb=create_or_load_vectorstore(invoice_docs, "order_pdfs")

order_retriever = order_vectordb.as_retriever(search_type="similarity", search_kwargs={"k": 4})
...
def general_agent(state: State):
    ...
    print(f"General agent {response}")
    return {"messages": [response]}
